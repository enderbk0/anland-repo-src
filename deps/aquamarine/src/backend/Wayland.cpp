...
int Aquamarine::CWaylandBackend::drmFD() {
    if (drmState.fd < 0) {
        drmState.fd = open("/dev/kgsl-3d0", O_RDWR | O_CLOEXEC);
        if (drmState.fd >= 0) {
            backend->log(AQ_LOG_WARNING, "CWaylandBackend: drmState.fd is invalid, opened /dev/kgsl-3d0");
        } else {
            drmState.fd = open("/dev/mali0", O_RDWR | O_CLOEXEC);
            if (drmState.fd >= 0) {
                backend->log(AQ_LOG_WARNING, "CWaylandBackend: drmState.fd is invalid, opened /dev/mali0");
            } else {
                drmState.fd = open("/dev/null", O_RDWR | O_CLOEXEC);
                if (drmState.fd >= 0) {
                    backend->log(AQ_LOG_WARNING, "CWaylandBackend: drmState.fd is invalid, opened fallback /dev/null");
                }
            }
        }
    }
