...
int Aquamarine::CBackend::reopenDRMNode(int drmFD, bool allowRenderNode) {
    ...
    if (drmGetNodeTypeFromFd(drmFD) >= 0 && drmIsMaster(drmFD) > 0) {
    ...
    if (!name) {
        ...
        if (!name) {
            log(AQ_LOG_WARNING, "reopenDRMNode: drmGetDeviceNameFromFd2 failed, falling back to dup()");
            int newFD = fcntl(drmFD, F_DUPFD_CLOEXEC, 0);
            if (newFD < 0) {
                log(AQ_LOG_ERROR, "reopenDRMNode: dup failed");
                return -1;
            }
            return newFD;
        }
    ...
    if (drmIsMaster(drmFD) > 0 && drmGetNodeTypeFromFd(newFD) == DRM_NODE_PRIMARY) {
        drm_magic_t magic;
